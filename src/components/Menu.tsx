...
const Menu = () => {
  ...
  return (
    <div>
      {!open ? (
      <Image src='/open.png' alt='' width={25} height={20} onClick={()=>setOpen(true)}/>
      ):(
      <Image src='/close.png' alt='' width={25} height={20} onClick={()=>setOpen(false)}/>
      )}
      {open && (<div className='bg-red-500 text-white absolute left-0 top-24 w-full h-[calc(100vh-6rem)] flex flex-col gap-8 items-center justify-center text-3xl z-10'>
        ...
        <Link href='/orders' onClick={()=>setOpen(false)}>pedidos</Link>
        )}
